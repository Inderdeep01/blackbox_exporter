...
func ProbeHTTP(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, logger *slog.Logger) (success bool, responseBody []byte, responseHeaders http.Header) {
	var redirects int
	var (
	...
	if resp == nil {
		...
		if httpConfig.IncludeResponseHeaders {
	...
	return success, responseBody, responseHeaders
}
